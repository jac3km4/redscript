...
impl<'a> Decompiler<'a> {
    ...
    fn consume_intrisnic_typed(&mut self, op: Intrinsic, type_args: Vec<TypeName>) -> Result<Expr<SourceAst>, Error> {
        ...
        Ok(Expr::Call(
            Expr::Ident(Ident::from_static(op.into()), Span::ZERO).into(),
            (),
            type_args.into(),
            params.into(),
            (),
    ...
    fn consume_call(&mut self, name: &'static str, param_count: usize) -> Result<Expr<SourceAst>, Error> {
        ...
        Ok(Expr::Call(
            Expr::Ident(Ident::from_static(name), Span::ZERO).into(),
            (),
            [].into(),
            params.into(),
            (),
    ...
    fn consume_with(
        ...
        let res = match self.code.pop()? {
            ...
            &Instr::Construct(n, class) => {
                ...
                Expr::New(
                    TypeName::without_args(self.pool.def_name(class)?),
                    params.into(),
                    Span::ZERO,
                )
            }
            &Instr::InvokeStatic(_, _, idx, _) => {
                ...
                let expr = if let Some(ctx) = context {
                    Expr::Member(ctx.into(), name, Span::ZERO)
                    ...
                    if def.parent.is_undefined() {
                        if name.as_ref().starts_with("Cast;") {
                            ...
                            return Ok(Expr::Call(
                                Expr::Ident(name, Span::ZERO).into(),
                                (),
                                [type_name].into(),
                                params.into(),
                                (),
                        ...
                        Expr::Ident(name, Span::ZERO)
                    } else {
                        let class_name = self.pool.def_name(def.parent)?;
                        Expr::Member(Expr::Ident(class_name, Span::ZERO).into(), name, Span::ZERO)
                    }
                } else {
                ...
                Expr::Call(expr.into(), (), [].into(), params.into(), (), Span::ZERO)
            ...
            &Instr::InvokeVirtual(_, _, idx, _) => {
                ...
                let expr = if let Some(ctx) = context {
                    ...
                    Expr::Member(Expr::This(Span::ZERO).into(), name, Span::ZERO)
                };
                Expr::Call(expr.into(), (), [].into(), params.into(), (), Span::ZERO)
            }
            ...
            &Instr::DynamicCast(typ, _) => {
                let type_name = TypeName::from_repr(&self.pool.def_name(typ)?);
                let expr = self.consume()?;
                Expr::DynCast(type_name, Box::new(expr), Span::ZERO)
            }
